...
//#define rtcm_buf_size    1550
//uint8_t rtcm[rtcm_buf_size]={'\0'};
int i, j = 0;
...
HAL_StatusTypeDef Send_AT_command(const char *cmd, uint8_t *response,
...
void forward_to_rover(uint8_t byte) {
	HAL_UART_Transmit(&huart2, &byte, 1, HAL_MAX_DELAY);
}

void process_qird(uint8_t *buf, uint16_t len) {
	static int header_match = 0;

	for (int i = 0; i < len; i++) {
		uint8_t c = buf[i];

		// Skip "+QIRD:" lines (ASCII until newline)
		if (!header_done) {
			// Look for end of HTTP header: "\r\n\r\n"
			if (c == "\r\n\r\n"[header_match]) {
				header_match++;
				if (header_match == 4) {
					header_done = true;   // header complete
				}
			} else {
				header_match = 0;
			}
		} else {
			// After header is done, ignore trailing "OK\r\n"
			// Detect "OK" sequence
			if (c == 'O' && i + 1 < len && buf[i + 1] == 'K') {
				// Skip "OK\r\n"
				i += 3; // advance past "OK\r\n"
				continue;
			}

			// Forward RTCM binary to GNSS rover
			forward_to_rover(c);
		}
	}
}
uint16_t count_digit(uint16_t digit) {
	uint16_t count = 1;
	while (digit) {
		digit /= 10;
		if (digit != 0) {
...
void process_qird1(uint8_t *buf, uint16_t len) {
	static int state = 0;
	// 0 = searching for QIRD header
	// 1 = skipping digits until "\r\n" after QIRD
	// 2 = binary RTCM area
	...
	for (uint16_t i = 0; i < len; i++) {
		...
		// ------------------------------------------------------
		case 0:
			// Wait until we see "+QIRD:"
			if (c == '+') {
				// Check if starts with "+QIRD:"
				if (i + 6 < len && buf[i] == '+' && buf[i + 1] == 'Q'
						&& buf[i + 2] == 'I' && buf[i + 3] == 'R'
						&& buf[i + 4] == 'D' && buf[i + 5] == ':') {
					state = 1;     // skip number until CRLF
				}
			}
			break;

		case 1:
			// Skip digits "1262" until we see "\r\n"
			if (c == '\n') {
				state = 2; // raw RTCM starts after this
			}
			break;

			// ------------------------------------------------------
			// 2) RTCM Binary Stream
			// Skip trailing "OK\r\n"
			// ------------------------------------------------------
		case 2:
			if (c == 'O' && i + 3 < len && buf[i + 1] == 'K'
					&& buf[i + 2] == '\r' && buf[i + 3] == '\n') {
				i += 3;       // skip "OK\r\n"
				continue;
			}

			// Otherwise → forward binary byte
			forward_to_rover(c);
			break;
		}
	}
...
void process_stream(uint8_t *buf, uint16_t start, uint16_t stop) {
...
//	memset(rx_buf, 0, sizeof(rx_buf));
	...
	HAL_UART_Transmit(&huart1,
			"GET /test2 HTTP/1.1\r\nAuthorization: Basic YWRtaW46UEFOQGJ4OTky\r\nUser-Agent:NTRIP quectel\r\n\r\n",
			strlen("GET /test2 HTTP/1.1\r\nAuthorization: Basic YWRtaW46UEFOQGJ4OTky\r\nUser-Agent:NTRIP quectel\r\n\r\n"),2000);

	//	uint8_t ctrlz = 0x1A;   // Ctrl+Z character
//	HAL_UART_Transmit(&huart1, &ctrlz, 1, 100);
//
//	HAL_UART_Receive(&huart1, ota_rx_buf, 30, HAL_MAX_DELAY);
//	HAL_UART_Transmit(&huart2, ota_rx_buf, strlen(ota_rx_buf), 1000);
...
//	//	HAL_UART_Receive(&huart1, ota_rx_buf, 1550, HAL_MAX_DELAY);
//	sscanf((char*) ota_rx_buf, "\r\n+QIRD: %d", &filesize);
//	if (filesize > 0) {
//		process_qird(ota_rx_buf, RxLen);
//	}
	/* USER CODE END 2 */
...
//		HAL_UART_Transmit(&huart1, "AT+QIRD=0,1500\r\n",
//				strlen("AT+QIRD=0,1500\r\n"), 1000);
//		HAL_UARTEx_ReceiveToIdle(&huart1, ota_rx_buf, raw_rx_buf_size - 1,
//				&RxLen, HAL_MAX_DELAY);
//		sscanf((char*) ota_rx_buf, "\r\n+QIRD: %d", &filesize);
//		digit = count_digit(filesize);
//
//		if (filesize > 0) {
//
//			process_stream(ota_rx_buf,(11+digit), (RxLen-8));
//
//
//		}
...
void check_status() {
	if (status == HAL_ERROR) {
		SerialWrite(
				"AT response is wrong or something went wrong. Reset again\n");
...
bool is_app_flash_erased(void) {
	for (uint32_t addr = FLASH_APP_ADDR; addr < FLASH_APP_ADDR + APP_SIZE_BYTES;
			addr += 4) {
...
void perform_OTA() {
	...
	if (strstr((char*) ota_rx_buf, "+QIACT: 1,1,1,\"") == NULL) {
		...
		HAL_Delay(1000);
		status = Send_and_read_AT_Command_("AT+QIACT?", "+QIACT: 1,1,1,\"", 5,
				1000);
	...
	status = Send_and_read_AT_Command_("AT+QHTTPURL=74,200", "\r\nCONNECT\r\n",
			5, 2000);
	check_status();
	HAL_Delay(1000);
...
//	status =Send_and_read_AT_Command_("https://raw.githubusercontent.com/kishan-shivhare/ota/main/f103_application.bin","\r\nOK\r\n", 5, 2000);
	status =
			Send_and_read_AT_Command_(
					"https://stm32-ota-firmware.s3.us-west-2.amazonaws.com/f103_application.bin",
					"\r\nOK\r\n", 5, 2000);
	check_status();
	HAL_Delay(1000);
	...
	status = Send_and_read_AT_Command_(
			"AT+QHTTPREADFILE=\"UFS:application.bin\",400", "\r\nOK\r\n", 5,
			2000);
	check_status();
	HAL_Delay(1000);

	SerialWrite(
			"Check if any file not closed previously, closing the opened file \n");
	status = Send_and_read_AT_Command_("AT+QFCLOSE=1027", "\r\nOK\r\n", 1,
			2000);
	HAL_Delay(1000);

	status = Send_and_read_AT_Command_("AT+QFLST=\"UFS:application.bin\"",
			"\r\n+QFLST: \"UFS:application.bin\",", 1, 2000);
	check_status();
	HAL_Delay(2000);

	sscanf((char*) ota_rx_buf, "\r\n+QFLST: \"UFS:application.bin\",%d",
			&filesize);
	...
	if (filesize && filehandle) {
		...
		SerialWrite(
				"file not found for OTA, closing the opened file and jumping to the existing application\n");
	...
	for (uint32_t i = 0; i < filesize; i += file_chunk_size) {
		...
		for (uint32_t k = 0; k < actual_data_len; k += 4) {
			...
			HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, FLASH_APP_ADDR + i + k,
					word);
		}
